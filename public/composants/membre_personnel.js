...
class Repertoire_agence extends React.Component {
    ...
    async update_agence(event) {
        ...
        if(res == 'r'){
            ...
            if(res =='deconnexion'){
                window.location.replace("https://agence.finuseco.com/")
            }else{
                let err = send.result,
                    erreurs=[]
                for (let i = 0; i < err.length; i++) {
            ...
            res.map((item)=>{
                if(item.etat_us == 1){
                    ...
                    this.setState({
                        ...
                        img             :'https://agence.finuseco.com/'+ item.photo,
                        img_replace     :'https://admin.finuseco.com/'+ item.photo,
    ...
    async send() {
        ...
        .then(async (element) => {
            ...
            if(check_mdp.result == 1){
                ...
                this.setState({state_mdp: false,state_btn:false});
            }else{
                swal({
                    title: "Erreur",
                ...
                this.setState({ state_msg :'',state_btn:false})
            }
        });
    }
    render() {
                ...
                <div className="card form-card" style={stylefrom}>
                    <div className="card-body">
                            ...
                            <div className="form-group m-b-0 text-right block_btn">
                                <button className="btn" disabled={this.state.state_btn} style={blockbtn} onClick={this.send}><i class="fas fa-save"></i>&nbsp;Déposer</button>
                            </div>
                    </div>
                </div>
            </div>
    ...
    async search_reference(event) {
        ...
        .then(async (element) => {
            ...
            if(check_mdp.result == 1){
                ...
                if(transct.result == 'deconnexion') window.location.replace("https://agence.finuseco.com/")
                else{
                    ...
                    <div className="card-body">
                        <table className="table table-striped" id="example">
                            <thead style={styleentete}>
                                ...
                                <Cellule_tableau label="Devise" />
                                <Cellule_tableau label="Montant" />
                                <Cellule_tableau label="Frais (en %)" />
                            </thead>
                            <tbody>
...
modifier_profil.addEventListener('click',async (e)=>{
    ...
    if(res_profil =='deconnexion') window.location.replace("https://agence.finuseco.com/")
...
if(rep_compte){
    rep_compte.addEventListener('click',async (e)=>{
        ...
        if(res_type_piece =='deconnexion') window.location.replace("https://agence.finuseco.com/")
