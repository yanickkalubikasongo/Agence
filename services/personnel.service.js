...
module.exports={
                               ...
                               /* if(await this.check_pseudo_membre(data.prenom)==''){
                                ...
                                let code_us=''
                                 ...
                                 let numPhone= user.dataValues.telephone,
                                    message =`Votre compte vient d'être crée avec succès chez Finuseco pseudo: ${part_membre.dataValues.pseudo} mdp : ${mdp_user} Vous aurez un IM lorsque vous serez affecté à une structure Veuillez changer votre mot de passe pour le sécuriser. Merci.`,
    ...
    check_state_user: async (data)=>{ 
        const [check_state_user, meta] = await sequelize.query("SELECT employes.etat_user FROM employes WHERE employes.id_user=:id_user",
        {
            replacements: { id_user:data.id }
        }),
        check_state_user_ = JSON.parse(JSON.stringify(check_state_user))
    ...
    on_personnel : async (data)=>{ 
        ...
        if(data.type == 6){
            ...
            const [on_personne, meta] = await sequelize.query("SELECT users.*,agences.id id_agence,agences.denomination,employes.id_type_employe id_type,employes.etat_user eta FROM users INNER JOIN employes ON employes.id_user=users.id INNER JOIN agences ON agences.id =employes.id_agence INNER JOIN type_employes ON type_employes.id=employes.id_type_employe WHERE users.id=:id",
            {
                replacements: { id:data.id }
            })
            on_personne_ = JSON.parse(JSON.stringify(on_personne)) 
