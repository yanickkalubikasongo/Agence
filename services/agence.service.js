        ...
        try{
            ...
            if(data.state_otp != true){
                ...
                let numPhone1=id_compte_exp[0].telephone,
                    message1=`Cher membre, veuillez confirmer votre transfert de : ${data.montant} votre OTP est le suivant: ${transfert_otp.dataValues.otp}`,
                    url1 = `https://api2.dream-digital.info/api/SendSMS?api_id=API12506297241&api_password=x3McB19tzd&sms_type=T&encoding=T&sender_id=FINUSECO&phonenumber=${numPhone1}&textmessage=${message1}`;
                                ...
                                if(parseFloat(await this.solde_membre(compte_exp)) > parseFloat(montant_plus_frais)){ // Vérification solde
                                    ...
                                    /* OTP Expéditeur */
                                        ...
                                        let numPhone=id_compte_exp[0].telephone,
                                            message=`Transfert effectue sur ${id_compte_rec[0].prenom+' '+id_compte_rec[0].nom} Montant : ${await this.formatage_depot(trans.dataValues.montant_envoye) + ' '+data.devise} Compte:  ${await this.formatage_compte(id_compte_rec[0].compte)} Frais: ${await this.formatage_frais((trans.dataValues.montant_envoye * trans.dataValues.pourc_preleve)) + ' '+data.devise} Solde : ${await this.formatage_solde(parseFloat(await this.solde_membre(compte_exp))) +' '+trans.dataValues.devise} Ref : ${await this.get_formate_ref_opertion(trans.dataValues.id,'T') + trans.dataValues.ref_operation} 
                                            Merci.`,
                                    ...
                                    /* OTP Recepteur */
                                        ...
                                        let numPhone2=id_compte_rec[0].telephone,
                                            message2=`Vous avez reçu de ${id_compte_exp[0].prenom+' '+id_compte_exp[0].nom} ${await this.formatage_depot(trans.dataValues.montant_envoye) + ' '+data.devise} Compte:  ${await this.formatage_compte(id_compte_exp[0].compte)} Solde : ${await this.formatage_solde(await this.solde_membre(compte_rec)) +' '+trans.dataValues.devise} Ref : ${await this.get_formate_ref_opertion(trans.dataValues.id,'T') + trans.dataValues.ref_operation} Merci.`,
                                        url2 = `https://api2.dream-digital.info/api/SendSMS?api_id=API12506297241&api_password=x3McB19tzd&sms_type=T&encoding=T&sender_id=FINUSECO&phonenumber=${numPhone2}&textmessage=${message2}`;
            ...
            console.log("Message transfert *****************************************************"+ msg)
            return msg
        }catch (error) {
            res.send('Erreur '+ error)
        }
        ...
        try{
            ...
            if(data.state_otp != true){
                ...
                let numPhone=id_compte[0].telephone,
                    message=`Cher membre, veuillez confirmer votre retrait de : ${data.montant+ ' '+data.devise} votre OTP est le suivant: ${retrait_otp.dataValues.otp}`,
                    url = `https://api2.dream-digital.info/api/SendSMS?api_id=API12506297241&api_password=x3McB19tzd&sms_type=T&encoding=T&sender_id=FINUSECO&phonenumber=${numPhone}&textmessage=${message}`;
                await axios.get(url);
                        ...
                        if(parseFloat(montant_min[0].f) <= parseFloat(data.montant)){
                            ...
                            if(parseFloat(await this.solde_membre(data)) > parseFloat(montant_plus_frais)){
                                ...
                                if(retrait.dataValues.ref_operation){
                                    let numPhone=id_compte[0].telephone,
                                        message =`Retrait effectue sur ${' '+id_compte[0].prenom+ ' '+id_compte[0].nom} Compte : ${compte_converti} Montant : ${retrait_formate + ' '+data.devise} Frais : ${frais_converti + ' '+data.devise} Solde : ${solde_formate + ' '+data.devise} Ref : ${await this.get_formate_ref_opertion(retrait.dataValues.id,'R')} Merci.`,
                                        url = `https://api2.dream-digital.info/api/SendSMS?api_id=API12506297241&api_password=x3McB19tzd&sms_type=T&encoding=T&sender_id=FINUSECO&phonenumber=${numPhone}&textmessage=${message}`;
                                    await axios.get(url);
                                    msg='r'
                               }
                ...
                if(parseFloat(montant_min[0].f) <= parseFloat(data.montant)){
                    if((await this.solde_monnaie_electronique_agence(agence,data.devise)  >= parseFloat(data.montant))){ // vérification solde
                        ...
                        if(depot.dataValues.ref_operation){
                            let numPhone=id_compte[0].telephone,
                                message =`Depot effectue sur ${' '+id_compte[0].prenom+ ' '+id_compte[0].nom}  Compte : ${compte_converti} Montant : ${depot_formate + ' '+data.devise} Solde : ${solde_formate + ' '+data.devise} Ref : ${ await this.get_formate_ref_opertion(depot.dataValues.id,'D' ) } Merci.`,
                                url = `https://api2.dream-digital.info/api/SendSMS?api_id=API12506297241&api_password=x3McB19tzd&sms_type=T&encoding=T&sender_id=FINUSECO&phonenumber=${numPhone}&textmessage=${message}`;
                            await axios.get(url);
                            msg='r'
                        }
