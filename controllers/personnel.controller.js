...
module.exports={
    ...
    enreg_personnel: async (req, res)=> {
        if(typeof req.session.org !== 'undefined' && typeof req.session.pseudo !== 'undefined' && typeof req.session.mdp !== 'undefined' && typeof req.session.id_compte !== 'undefined')  {
            console.log("organisation ****************************************************************************"+ req.session.org)
            try { 
                let file = ''
                                                                                                                ...
                                                                                                                if(helperServices.presence(dataRequest.etat_civil)) erreur.push('Merci de choisir un Etat-civil')     
                                                                                                                    ...
                                                                                                                    if(helperServices.presence(dataRequest.numero_piece)) erreur.push('Merci de remplir le champ Numéro pièce d\'identité') 
                                                                                                                        ...
                                                                                                                        if(helperServices.content_letter(dataRequest.adresse)) erreur.push('L\'adresse physique doit comporter au moins une lettre ')  
                                                                                                                            ...
                                                                                                                            if(await personnelServices.check_email(dataRequest.email) !='' ) erreur.push('Cette adresse email a déjà été utilisée ')
                                                                                                                            else{
                                                                                                                                if(await personnelServices.check_telephone(dataRequest.telephone) !='' ) erreur.push('Ce numéro de téléphone a déjà été utilisé')
                                                                                                                                else{
                                                                                                                                    if(helperServices.valid_telephone(dataRequest.telephone)) erreur.push('Le numéro de téléphone doit êre saisi sous format +243XXXXXXXXX')
                                                                                                                                }
                                                                                                                            }
                                                                                                                        }    
                                                                                                                    }
    ...
    rep_personnel: async (req, res)=> {
        if(typeof req.session.org !== 'undefined' && typeof req.session.pseudo !== 'undefined' && typeof req.session.mdp !== 'undefined' && typeof req.session.id_compte !== 'undefined')  {
            try {
                res.json({
                  result: await personnelServices.rep_personnel(req.body,req.session.org)
            });
            }catch (error) {
                res.send('Erreur '+ error)
            }
